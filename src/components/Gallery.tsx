...
function Cube() {
  ...
  return (
    <div className="mx-auto mt-8 grid h-24 w-24 place-items-center" style={{ perspective: "400px" }}>
      <div className="gpu relative h-full w-full" style={{ transformStyle: "preserve-3d", animation: "cube-spin 12s linear infinite" }}>
        {faces.map((f, i) => (
          <div
            key={i}
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              {shots.map((s, i) => (
                <Reveal key={s.cap} delay={i * 90} className={cn("min-h-0", s.cls)}>
                  <figure className="webcore-tile overflow-hidden  transition-colors duration-300">
